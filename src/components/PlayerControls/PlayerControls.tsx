...
export function PlayerControls({
    ...
    const [currentSlidingValue, setCurrentSlidingValue] = useState(currentTime / 10000 * duration);

    // Making sure to when the slider is sliding so we don't get weird glitches and is good UX wise
    const [isSliding, setIsSliding] = useState(false);

    useEffect(() => {
        if(!isSliding) {
            setChangingTime(currentTime);
            setCurrentSlidingValue(currentTime / 10000 * duration);
        }
    ...
    return (
        <Wrapper visible={controlsVisible}>
            {controlsVisible &&
                <Fragment>
                    <PlayPause>
                        <Layout.Block direction='row' align='center' justify='space-between' width={300}>
                    ...
                    <SliderWrapper>
                        <Typography.Description color='white'>{secondsToHms(changingTime)}</Typography.Description>
                        <Slider
                        ...
                        />
                        <Typography.Description color='white'>{secondsToHms(duration)}</Typography.Description>
                    </SliderWrapper>
                </Fragment>
            }
